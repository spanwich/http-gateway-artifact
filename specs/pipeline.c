/*
 * Batch C: Multi-Session E2E Pipeline Implementation
 *
 * Adapted from batch_a/priority_1_2_rbac/rbac_app.c.
 * Adds session lookup and extraction stages before EverParse validation.
 *
 * Three layers:
 *   1. This glue code: session lookup, extraction, buffer construction, dispatch
 ...
 * Phase 5: rules are 8 bytes (path_hash:4 + method:1 + min_role:1 + req_scope:2)
 */
...
void policy_init_default(Policy *p)
{
    ...
    /* Rule 0: GET /status requires OPERATOR, no scope */
    p->rules[0] = (PolicyRule){ PATH_STATUS, METHOD_GET, ROLE_OPERATOR, 0x0000 };

    /* Rule 1: POST /logout requires OPERATOR, no scope */
    p->rules[1] = (PolicyRule){ PATH_LOGOUT, METHOD_POST, ROLE_OPERATOR, 0x0000 };

    /* Rule 2: GET /policy requires ADMIN, no scope */
    p->rules[2] = (PolicyRule){ PATH_POLICY, METHOD_GET, ROLE_ADMIN, 0x0000 };
 ...
 * LoginRequest is UNCHANGED in Phase 5 (no scope).
 */
static RequestResult handle_login(SessionEntry *se, const SecurityParams *params)
{
...
/*
 * Process PUT /policy
 * body format: [num_rules:1][rule_0:8][rule_1:8]...[rule_7:8]
 * Total body: 1 + 64 = 65 bytes
 */
static RequestResult handle_policy_update(SessionEntry *se, Policy *p,
                                          const SecurityParams *params)
{
    const uint8_t *body = params->body;
    uint32_t body_len = params->body_len;

    if (body == NULL || body_len < 65) {
        return RESULT_DENIED;
    }
 ...
 *
 * Standalone test uses auth_scope=0xFFFF (all scopes granted) since
 * there is no Authenticator in standalone mode.
 */
static RequestResult handle_access_request(SessionEntry *se, const Policy *p,
                                           const SecurityParams *params)
{
    ...
    buf[0] = se->auth_state;
    buf[1] = se->rate_count;
    write_u16(buf, 2, 0xFFFF);  /* auth_scope: all scopes (standalone) */
...
{
    /* Step 1: Session lookup/create */
    SessionEntry *se = session_lookup(sessions, client_id);
    if (!se) {
        se = session_create(sessions, client_id);
        if (!se) return RESULT_NO_SESSION;
    }

    /* Step 2: Extraction */
    SecurityParams params;
    if (mock_extract(req, &params) != EXTRACT_OK) {
        return RESULT_EXTRACTION_FAILED;
    }

    /* Step 3: Dispatch */
    if (params.path_hash == PATH_LOGIN && params.method == METHOD_POST) {
        return handle_login(se, &params);
    }

    if (params.path_hash == PATH_POLICY && params.method == METHOD_PUT) {
        return handle_policy_update(se, policy, &params);
    }

    return handle_access_request(se, policy, &params);
}
...
const char *result_name(RequestResult r)
{
    ...
    case RESULT_EXTRACTION_FAILED: return "EXTRACTION_FAILED";
    case RESULT_NO_SESSION:        return "NO_SESSION";
